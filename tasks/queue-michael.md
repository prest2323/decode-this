...
- [x] 3. **Multimodal extraction prompt + JSON schema** (`lib/prompt.ts`). Export `EXTRACT_SYSTEM` (string) and `extractSchema` mirroring `DocumentModel` for **constrained** generation. _(Done — deepened EXTRACT_SYSTEM (doc-class-general, grouping/difficulty/ordering rules, never-invent, 2 few-shots) + added `extractSchema` (Type enum) mirroring DocumentModel minus app-derived spotlight/status/value/pages. build green; `Type` resolves. Live extraction is exercised in task 4/8 once a key exists; few-shot tuning is task 13.)_ Instruct the model: return the 3-sentence plain `summary`, `docType`, `topFlags`, and a `Requirement[]`. For `@google/genai` build the schema with the `Type` enum, e.g.
  ```ts
...
- [x] 4. **Real analyzer: document → DocumentModel** (`lib/extract.ts` → `extractDoc(req: AnalyzeRequest): Promise<DocumentModel>`). Use Preston's provider client in `lib/ai.ts` (don't re-init keys). Send the file as **multimodal**: base64 in an `inlineData` part (`mimeType:"application/pdf"` for PDFs, `image/jpeg|png` for images). Verified Gemini call:
  ...
  ```
  (Strip any `data:...;base64,` prefix before sending.) Then **always** `return normalizeDoc(raw)`. If no key → return `MOCK_SBA`. Anthropic fallback uses `messages.parse({ output_config:{ format:{ type:"json_schema", schema } } })` with a `{type:"document",source:{type:"base64",media_type:"application/pdf",data}}` block and reads `message.parsed_output`. Wrap in try/catch → on any failure fall back to mock so the demo never dies. _(Done — extractWithGemini (default; multimodal inlineData + responseSchema=extractSchema), extractWithAnthropic (tool-use with input_schema; chose this over the unverified `messages.parse` for SDK-0.100 safety), a zero-requirements guard, and analyzeDocument routing by provider → normalizeDoc → mock on any failure. build typechecks both SDK calls; mock + `?demo=1` verified (9 reqs). ⚠ live extraction needs GEMINI_API_KEY (untested here). **FLAG @Preston:** the model doesn't return `pages` — the canvas must render the uploaded file into page images (rects are normalized to overlay whatever it draws). Who owns populating `DocumentModel.pages` for a live upload?)_

- [x] 5. **Grouping + difficulty pass** (`lib/extract.ts` → `groupAndOrder(reqs): Requirement[]`, run inside `normalizeDoc`). (a) **Group**: collapse adjacent `fill-field` requirements whose fields are the same logical unit (address: street+city+state+zip; full name parts) into ONE requirement with ONE `spotlight` = the union bounding box — the user explicitly demanded street+city+state+zip in a single spotlight. (b) **Score difficulty**: `gather-document`/`external-action`/`sign`/`pay-fee` skew harder; a lone text field is `easy`. (c) **Order the tour**: prerequisites (gather/external) before on-page fills; within that, easy → hard so the user warms up. Set each `order` to the final index. Unit-eyeball it against the SBA mock. _(Done — runs inside normalizeDoc: (a) merges consecutive same-unit (address/name) fill-fields → ONE spotlight (safety net; idempotent on the already-grouped mock), (b) difficulty = max(provided, type-baseline, flag-severity), (c) orders prereqs→fills(easy→hard, top→bottom)→pay-fee→sign-last. Live path verified: taxreturn,getein,legalname,address,loan,ein,ownership,fee,sign; address stays ONE req. FYI @Preston/@Aiden: `?demo=1` returns MOCK_DOC RAW (authored order), so the rehearsed hero keeps its order; only live uploads get re-ordered — say if you want them identical.)_

- [x] 6. **Grounded chat** (`lib/chat.ts` → `answer(req: ChatRequest): Promise<ChatResult>`). Context-stuff: pass `doc.summary` + `doc.docType` + every requirement's `title`/`guidance` + (if `activeRequirementId`) the **active** requirement verbatim into the model. Answer in `req.lang` (bilingual-aware), **never invent doc facts** — if it's not in the doc, say so and suggest who to ask. Return `{ answer, citedRequirementIds }` populated from which requirements you grounded on (Aiden's `ChatWidget` highlights them). No key → a deterministic mock answer keyed off the active requirement so the chat demos offline. _(Done — chatAnswer routes by provider; real path context-stuffs docType+summary+every step's title/guidance+the active requirement into CHAT_SYSTEM and returns {answer, citedRequirementIds (active + keyword overlap, capped 4)}; grounded mock kept as the offline + failure fallback. Verified /api/chat mock: personal-guarantee→cites r_ownership, deadline(es), fee(≈$2,500), active=r_address→step guidance. build typechecks the real path; ⚠ live model needs a key.)_

- [x] 7. **Export: fill + multi-format** (`lib/export.ts` → `exportDoc(doc, format: ExportFormat): Promise<Blob>`). `npm i pdf-lib`. **PDF** (the money format): load the original PDF bytes, fill AcroForm values from each `Field.value`, flatten, save → verified pattern:
  ```ts
  ...
  ```
  Use `getFieldMaybe` everywhere so a name mismatch never throws. **JSON** = `JSON.stringify(doc)`. **CSV** = one row per `Field` (`requirement, field name, label.en, value`). `store.exportAs(format)` calls this; you return the Blob, the store triggers download. (DOCX is a stretch — stub it to throw a friendly "coming soon" and move on.) _(Done — JSON (full model), CSV (one row per field; checkbox→Yes), PDF (pdf-lib generates a clean filled-answers doc; verified valid %PDF bytes via a Node test of buildExport), DOCX friendly stub. Kept the store's void `exportDoc(doc,format,lang)` signature + split out a pure testable buildExport. **Added pdf-lib to package.json** (sanctioned by this task + AGENTS locked-libs). **FLAGS @Preston:** (1) `pdfjs-dist` is ALSO missing from package.json — Sawyer's canvas needs it. (2) Filling the ORIGINAL uploaded AcroForm needs the original PDF bytes in DocumentModel/store (not in the contract) — I generate a PDF from the model instead. (3) Spec wanted exportDoc→Promise<Blob>; I kept the void download signature your store already calls — say if you'd rather Blob-return.)_

- [x] 8. **Eval harness** (`scripts/eval.mjs`, NEW — `npm run eval` or `node scripts/eval.mjs`). POST each sample doc in `scripts/samples/` to `http://localhost:3000/api/analyze` and print a table: `{ doc, requirements_found, flags_found, fill_field_groups, grouping_ok?, latency_ms }`. `grouping_ok` = did street+city+state+zip collapse into ONE `fill-field` requirement? Pure Node `fetch`, zero deps. This is your regression net — run it after every prompt change so we catch drift before the judges do. _(Done — scripts/eval.mjs (pure Node, zero deps) POSTs scripts/samples/* to /api/analyze, prints requirements_found / flags_found / fill_field_groups / grouping_ok / latency_ms, exits 1 on failure or grouping drift. Added `npm run eval`. Verified vs the SBA mock: 9 reqs, 12 flags, 5 fill groups, grouping_ok=true, 74ms. Add real docs to scripts/samples/ + a key for live accuracy numbers.)_
